...
- GitHub Actions를 활용하여 코드 Push → 테스트 실행 → Docker 이미지 빌드 → ECR Push → ECS 배포까지 **자동화된 CI/CD 파이프라인을 구축**했습니다.
- **OIDC 기반 인증**을 적용하여 GitHub에 AWS Access Key를 저장하지 않고 안전하게 AWS 리소스에 접근하도록 설계했습니다.
